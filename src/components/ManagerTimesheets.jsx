...
const ManagerTimesheets = () => {
  ...
  useEffect(() => {
    const fetchSubmissions = async () => {
      try {
        const { data } = await axios.get(`https://webapp011-beemb2fbcuhahfbc.canadacentral-01.azurewebsites.net/api/timesheets/list/manager/${managerId}`);
        setSubmissions(data);
  ...
  const handleApprove = async (id) => {
    ...
    try {
      await axios.put(
        `https://webapp011-beemb2fbcuhahfbc.canadacentral-01.azurewebsites.net/api/timesheets/Approve/${id}/status/APPROVED`
      );
  ...
  const handleReject = async () => {
    ...
    try {
      await axios.put(
        `https://webapp011-beemb2fbcuhahfbc.canadacentral-01.azurewebsites.net/api/timesheets/reject/${currentId}/status/REJECTED/comments/${comments}`
      );
      fetchUpdatedSubmissions();
      handleClose();
    } catch (error) {
